...
-- Date:              Thu Jan 16 16:38:11 2014 (by Create and Import Peripheral Wizard)
...
begin
  ...
  begin
    -- REGISTER 0, 4, 5, 6, 7 are all read-only, therefore they are removed from this process
  ...
  SLAVE_REG_READ_PROC : process( slv_reg_read_sel, slv_reg0, slv_reg4, slv_reg5, slv_reg6, slv_reg7 ) is
  begin
    -- REGISTER 1, 2, 3 are write-only, therefore they are omitted from the sensitivity list and this process
  ...
  -- Normally all behaviours is taken care of by the controller itself, therefore no need to worry about additional signals
  -- Maybe this statement is wrong, but we'll take it as long as there is no problem
  
  modem_ctrl_inst: modem_controller port map(
    h => Bus2IP_Clk,
    rst => Bus2IP_Reset,
    dtr => slv_reg1(1),
    rts => slv_reg1(2),
